...
This vignette serves as a test for \verb|ucR::time_match|.
...
\item \verb|end| last date searched untill
...
\item \verb|alias| name of pattern searched for (else pattern)
...
kable(tm[, c('id', 'event', 'alias', 'match', 'match.in')])
@
\vspace{0.5cm}
