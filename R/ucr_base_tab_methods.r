# ' Wrapper of linear model to produce an overall P-value.
# ' @author Lars Lindhagen
# ' @param ... arguments to be passed to \code{\link{lm}}

ucr.internal.base.tab.lm <- function(...) {
   mod <- summary(lm(...))
   mod$p.value <- pf(mod$fstatistic[1], mod$fstatistic[2], mod$fstatistic[3],
      lower.tail = FALSE) # Code stolen from print.summary.lm
   return (mod)
}
...
ucr.internal.base.tab.parameter.check <- function(data, group.name,
   ...
   assert.string.in.set(num.test, c("nonparam", "param"))
   assert.string.in.set(factor.test, c("pearson", "fisher"))
