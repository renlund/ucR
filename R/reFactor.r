...
reFactor <- function(x,L, na.level=NULL, exclude=NULL, new.last=FALSE){
   ...
   for(key in nL){ 
      if(key %in% levels_cleared){
         warning("[reFactor] Key '",key,"' has been used previously and will be skipped.")
         next
      }
      a <- L[[key]] 
      if( !(key %in% lev) ){
         if( !is.null(a) ) warning(paste0("[reFactor] Key ", key, 
      ...
      if(any(vs %in% levels_cleared)){
         warning(paste0("[reFactor] Key '",key,
      ...
      if(sum(!filt)>1){
         warning(paste0("[reFactor] Multiple new names for level '",
